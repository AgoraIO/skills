...
`agora init` creates or binds an Agora project, clones the selected quickstart, writes its env file, persists repo-local project context, and prints next steps.

```bash
agora quickstart list
agora quickstart list --verbose
...
Verified `0.1.7` resolution order:
...
`.agora/project.json` can include durable metadata such as project ID, project name, region, template, and env path. Do not put secrets in this file.
